...
FRAME_THRESHOLD = 15  # Reduced from 20 to 15 for faster response
FRAME_CONSECUTIVE_THRESHOLD = 5  # Reduced from 10 to 5
CNN_PROB_THRESHOLD = 0.9
EAR_THRESHOLD = 0.3  # Default until baseline
MAR_THRESHOLD = 0.5  # Default until baseline
...
ear_buffer = deque(maxlen=7)  # Reduced from 10 to 7 for faster EAR response
mar_buffer = deque(maxlen=15)  # Kept at 15 to filter talking
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        # Trigger on low EAR alone, or low EAR with high MAR/CNN confirmation
    ...
    # Stop alarm if no drowsiness is detected
